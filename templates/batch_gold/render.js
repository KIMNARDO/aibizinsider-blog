...
484: {
 headline:"AI 투자 시장 격변 — Anthropic 바이오텍 4억 달러 인수, OpenAI 경영진 교체, 세컨더리 마켓 6000억 달러",
 bullets:[
...
function render(d) {
  ...
  const secs = d.sections.map(([h2,body,h3,h3b]) =>
    `<h2 style="font-size:28px;font-weight:700;color:#0f172a;border-bottom:3px solid #f59e0b;padding-bottom:10px;margin-top:40px;line-height:1.4;">${h2}</h2>`+
    `<p style="font-size:16px;color:#334155;line-height:1.9;">${body}</p>`+
    `<h3 style="font-size:20px;font-weight:700;color:#92400e;border-left:4px solid #fbbf24;padding-left:14px;margin-top:36px;">${h3}</h3>`+
    `<p style="font-size:16px;color:#334155;line-height:1.9;">${h3b}</p>`
  ).join('');
  const sources = d.sources.map(([l,u]) => `<p style="margin:0 0 6px;font-size:14px;color:#334155;"><a href="${u}" style="color:#f59e0b;" target="_blank" rel="noopener">${l}</a></p>`).join('');
  const related = d.related.map(([l,u]) => `<li><a href="${u}" style="color:#f59e0b;text-decoration:none;">${l}</a></li>`).join('');
  return `<!-- wp:html -->
<div style="max-width:720px;margin:0 auto;font-family:'Pretendard','Noto Sans KR',sans-serif;">
<div style="background:linear-gradient(135deg,#0a1628 0%,#1a2e4a 100%);border-radius:12px;padding:28px 32px;margin:32px 0;"><p style="margin:0 0 14px;font-size:13px;font-weight:700;letter-spacing:0.12em;color:#fbbf24;text-transform:uppercase;">TL;DR</p><p style="margin:0 0 12px;font-size:18px;font-weight:700;color:#fbbf24;line-height:1.4;">${d.headline}</p><ul style="margin:0;padding-left:20px;color:#e2e8f0;font-size:15px;line-height:1.9;">${bullets}</ul></div>
${secs}
<div style="background:#fef3c7;border:1px solid #f59e0b;border-radius:8px;padding:22px 24px;margin:24px 0;"><p style="margin:0 0 6px;font-size:13px;font-weight:700;letter-spacing:0.08em;color:#f59e0b;text-transform:uppercase;">AI Biz Insider 분석</p><p style="margin:12px 0 0;font-size:16px;color:#78350f;line-height:1.9;font-style:italic;">${d.analysis}</p></div>
...
fs.writeFileSync(path.join(__dirname, 'rendered.json'), JSON.stringify(out));
console.log(`Rendered ${Object.keys(out).length} posts`);
for (const pid of Object.keys(out)) {
  console.log(`  ${pid}: ${out[pid].length} chars`);
}
